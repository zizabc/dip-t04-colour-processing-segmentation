...
def img_k_means(img, pixel_mode, k, n, S=None):
    ...
    # Selecting centroids (with S as the seed)
    random.seed(S)
    ...
    # Iterating
    for i in range(n):
        dists = [np.linalg.norm(dataset - dataset[i]) for i in ids]
        # for x in range(img.shape[0]):
        #     for y in range(img.shape[1]):
        #         # Computing Euclidean distance to each centroid
        #         dists = np.array([np.linalg.norm(dataset[x*img.shape[0]+y] \
        #             - dataset[i]) for i in ids])
        #         img_ret[x, y] = np.argmin(dists) + 1
...
def rmse(img_i, img_r):
    ...
    return np.sqrt((1/(img_i.shape[0] * img_i.shape[1]))
                   * np.sum(np.square(img_i.astype(float)
                                      - img_r.astype(float))))

# x -> # np.repeat(np.arange(0, img_i.shape[0], dtype=int), img_i.shape[1]*img_i.shape[2]).shape
# y -> # np.array(list(np.repeat(np.arange(0, img_i.shape[1], dtype=int), img_i.shape[2]))*img_i.shape[0]).shape
...
def dataset_gen_from_img(img, pixel_mode):
    ...
    if(pixel_mode == 1):
        ...
        # Luminance
        dataset = np.array([0.299 * img[x, y, 0] + 0.587 * img[x, y, 1]
                            + 0.114 * img[x, y, 2]
                            for x in range(img.shape[0])
                            for y in range(img.shape[1])],
                           dtype=np.double)
...
if __name__ == "__main__":
    ...
    img_i, img_r = imageio.imread(
        ipath), imageio.imread(rpath)  # reading images
    ...
    print('%.4f' % rmse(img_k_means(img_i, pixel_mode, k, n, S), img_r))
